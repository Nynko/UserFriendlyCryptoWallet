...
import { CreateAnonCredsDef } from "../functions/createAnoncredsDef"
import { createCredential, createCredRequest } from "../functions/createCredentials";
import { createPresentation, verifyPresentation } from "../functions/createPresentations";
import { anoncreds, Credential, CredentialDefinition, CredentialOffer, CredentialRequest, CredentialRevocationConfig, LinkSecret, Presentation, PresentationRequest, RevocationRegistryDefinition, RevocationStatusList, Schema } from "@hyperledger/anoncreds-react-native";
...
// ZEZEZEZEZEZEZEZEZEZEZ
        ...
        const {schema, schemaId, credentialDefinition,credDefId,credentialDefinitionPrivate, keyCorrectnessProof} = CreateAnonCredsDef();
        console.log(schema);
        
        const {linkSecret, credentialOffer, credentialRequest,credentialRequestMetadata} = createCredRequest(schemaId,credentialDefinition,credDefId,keyCorrectnessProof)
        console.log("credRequest", credentialRequest.toJson());
        const credential = createCredential(linkSecret,credentialRequestMetadata,credentialDefinition,credentialDefinitionPrivate,credentialOffer,credentialRequest);

        console.log("credential",credential.toJson());

        const {presentation, presentationRequest} = createPresentation(credential,credDefId,linkSecret,schema,schemaId,credentialDefinition);
        console.log("presentation",presentation.toJson());

        const good = verifyPresentation(presentation,presentationRequest,schema,schemaId,credentialDefinition,credDefId);
